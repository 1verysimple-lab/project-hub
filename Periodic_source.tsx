...
import { Beaker, Droplet, Wind, Zap, Mountain, ShieldAlert, Radio, Crown, BookOpen, ChevronRight, CheckCircle2, Atom, Lock, Unlock, X, FlaskConical, Pill, Apple } from 'lucide-react';

// --- Types & Interfaces ---

type Category = 'alkali' | 'alkaline' | 'transition' | 'basic' | 'metalloid' | 'nonmetal' | 'halogen' | 'noble' | 'lanthanide' | 'actinide' | 'unknown';
...
const categories: Record<Category, { name: string; color: string; desc: string }> = {
...
// Compact data representation for all 118 elements to keep file size reasonable
// Format: "Atomic|Symbol|Name|Category"
const elementStringData = [
  ...
  // Lanthanides (57-71) inserted later
  ...
  // Actinides (89-103) inserted later
...
const generateFullTable = (): ElementData[] => {
  const elements: ElementData[] = [];
  
  // Custom descriptions map
  ...
  elementStringData.forEach(str => {
    ...
    // Logic to determine grid position based on atomic number
    ...
    else if (atomic >= 72 && atomic <= 86) { row = 6; col = atomic - 68; } // Skip gap
    else if (atomic >= 87 && atomic <= 88) { row = 7; col = atomic - 86; }
    else if (atomic >= 104 && atomic <= 118) { row = 7; col = atomic - 100; } // Skip gap
    // Lanthanides
    else if (atomic >= 57 && atomic <= 71) { row = 9; col = atomic - 54; } // Placed below row 7 (row 9 visually)
    // Actinides
...
export default function App() {
  ...
  // Filter elements based on level
  const isUnlocked = (atomic: number) => {
